...
/**
 * Utility methods for reading Spring/JPA annotations from JavaParser nodes.
 */
final class AnnotationSupport {
	...
	/**
	 * Checks whether a node has a given annotation by simple name.
	 */
	...
	/**
	 * Returns annotation names in the `@SimpleName` form.
	 */
	static List<String> annotationNames(NodeWithAnnotations<?> node) {
		return node.getAnnotations().stream().map(annotation -> "@" + annotation.getName().getIdentifier()).toList();
	}

	/**
	 * Resolves request mapping path from mapping annotation value/path.
	 */
	...
	/**
	 * Resolves HTTP method from mapping annotations.
	 */
	...
	/**
	 * Resolves JPA table name from `@Table(name = ...)`.
	 */
	...
	/**
	 * Resolves one annotation attribute value by annotation and key names.
	 */
	static Optional<String> resolveAnnotationValue(NodeWithAnnotations<?> node, String annotationSimpleName,
			String key) {
		for (AnnotationExpr annotation : node.getAnnotations()) {
	...
	/**
	 * Returns true when annotation is one of supported Spring mapping annotations.
	 */
	private static boolean isMappingAnnotation(String simpleName) {
		return "RequestMapping".equals(simpleName) || "GetMapping".equals(simpleName)
				|| "PostMapping".equals(simpleName) || "PutMapping".equals(simpleName)
				|| "DeleteMapping".equals(simpleName) || "PatchMapping".equals(simpleName);
	}

	/**
	 * Resolves string-like annotation attribute value.
	 */
	...
	/**
	 * Converts a JavaParser expression into a plain string representation.
	 */
