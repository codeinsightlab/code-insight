...
class ProjectScanControllerWebTest {
  ...
  @Mock private ProjectScanner projectScanner;

  @Mock private ProjectStructureScanner projectStructureScanner;

  @BeforeEach
  void setUp() {
    ProjectScanController controller =
        new ProjectScanController(projectScanner, projectStructureScanner);
    mockMvc =
  ...
  void shouldReturnProjectReportWhenScanApiCalled() throws Exception {
    given(projectScanner.scan("/tmp/demo"))
        .willReturn(new ProjectReport(2, 5, 1, 12));
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.data.controllerCount").value(2))
        .andExpect(jsonPath("$.data.endpointCount").value(5))
        .andExpect(jsonPath("$.data.entityCount").value(1))
